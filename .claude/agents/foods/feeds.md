...
You read and write `data/json/foods/feeds.json` and `data/json/sources.json`. Do not write any other file. When adding a new source, append it to sources.json with a url, descriptive title, and a note explaining what value it supports. Read `data/json/pesticides.json` to look up pesticide ids and names for the `pesticides` array. Schema questions go to `db-schema`; data auditing to `food-auditor`.

Current foods: alfalfa, grass-hay. These are not human foods (human_food=0) — they exist because animal feed arrays in the meat, dairy, seafood, and eggs files reference them by `food_id` and `food_slug`. All plant environmental metrics still apply since these crops have real land, water, and emissions footprints.
