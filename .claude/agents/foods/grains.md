...
You read and write `data/json/foods/grains.json` and `data/json/sources.json`. Do not write any other file. When adding a new source, append it to sources.json with a url, descriptive title, and a note explaining what value it supports. Read `data/json/pesticides.json` to look up pesticide ids and names for the `pesticides` array. Schema questions go to `db-schema`; data auditing to `food-auditor`.

Current foods: wheat, corn, sorghum, rice, oats, quinoa, barley. Note: corn also appears as an animal feed ingredient — the `food_id` and `food_slug` in animal feed arrays reference this file's entries.
