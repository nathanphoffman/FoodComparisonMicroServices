                        ...
                        <Row key={food.slug} className={food.slug === 'your-meal' ? 'bg-yellow-50' : undefined}>
                            {activeCols.map(column => {
                                switch (column.key) {
                                    ...
                                    case 'landUse':        return <LandUseCell        key="landUse"        value={scoredRow?.land_use ?? null} detail={scoredRow?.land_use_detail ?? { type: food.type, yieldKilogramsPerHectare: null, pastureHectaresPerKilogram: null, feedLandM2PerKg: null, rawM2PerKg: 0, landTypes: null, multiplier: 1 }} divisor={1} unit={unit} />;
                                    case 'directKill':     return <IntelligenceCell   key="directKill"     value={scoredRow?.direct_kill ?? null} detail={toIntelligenceDetail(food)} killDetail={scoredRow?.kill_detail} explanation={food.sentient_harm_explanation} />;
